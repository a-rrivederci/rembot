'''
 @title util.py
 @brief Rembot utility classes
 @authors DevBhullar AlvinHo Eeshiken
'''
...
class Camera:
    ...
    def take_picture(self):
        with picamera.PiCamera() as camera:
            ...
            # Camera warm-up time
            time.sleep(2)
            camera.capture('images/capture.jpg', resize=(400,250))
...
class Bot:
    ...
    def start_processing(self, imgpath):
        self.img.img_process(imgpath)
        return

    def bot_gen_cmds(self):
        self.img.generate_cmds()
        return

    def bot_send_cmds(self):
        self.img.send_serial()
        return
...
class Image:
    ...
    def img_process(self, imgpath):    
        ...
        #         painting[row_painting, col_painting] = self.cmyk_to_bgr(self.cyan_avg[row_painting, col_painting],
        #                                                         self.magenta_avg[row_painting, col_painting],
        #                                                         self.yellow_avg[row_painting, col_painting],
        #                                                         self.black_avg[row_painting, col_painting])
    ...
    def send_serial(self):
        ...
        while index != (self.s_cmds.shape)[0]:
            cmdValue = self.s_cmds[index]

            if cmdValue[0] < 0:
                port.write(cmdValue[0])
            else:
                port.write(cmdValue[0])
                port.write(cmdValue[1])
            
            read_serial = port.read(1)
            while port.read(1) != "y":
                pass
            progress = int(float(index/((self.s_cmds.shape)[0])) * 100)
            index += 1
